...
start() {
  ...
  for _ in $(seq 1 60); do
    # mysqladmin lives inside the container, so this needs no local client.
    if docker exec "$CONTAINER" mysqladmin ping -h 127.0.0.1 -u root --silent >/dev/null 2>&1; then
      echo " ok"
      return 0
    fi
    echo -n "."
