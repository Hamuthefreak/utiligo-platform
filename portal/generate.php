  ...
  <div class="glass rounded-2xl p-6 border border-white/5">
    <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
      ...
      <input type="search" id="templateSearch" placeholder="Search templates…"
             class="w-full sm:w-56 bg-slate-800/80 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-white/40 transition">
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const tplSearch = document.getElementById('templateSearch');
  if (tplSearch) {
    tplSearch.addEventListener('input', () => {
      const q = tplSearch.value.trim().toLowerCase();
      document.querySelectorAll('.template-card').forEach(card => {
        const hay = ((card.dataset.label || '') + ' ' + (card.dataset.description || '')).toLowerCase();
  ...
  function buildPreviewHTML(key) {
    ...
    // Live preview should reflect what the user typed, not placeholder copy.
    const bizName = (document.querySelector('input[name="business_name"]')?.value || 'Your Business').trim();
    const bizCat  = (document.querySelector('input[name="business_category"]')?.value || t.category).trim();
    const isDark    = t.dark;
