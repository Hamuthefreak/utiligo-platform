...
?>
<!DOCTYPE html>
<?php /* The dock window is the same surface as the in-portal dock, so it loads the
         same design layer in the same order — call_scripts.css reads the theme's
         tokens, and a window that did not load theme.css would draw the dock in
         fallback colours while the portal beside it was themed. */ ?>
...
<style>
  html, body { margin: 0; padding: 0; height: 100%; background: var(--canvas, var(--canvas)); }
