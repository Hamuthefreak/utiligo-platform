...
#sbPages .section-label { font-size: 9px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: #334155; padding: 0 6px; margin-bottom: 6px; }
.page-item {
...
#sbTools .section-label { font-size: 9px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: #334155; padding: 0 6px; margin-bottom: 6px; }
.tool-btn {
  display: flex; align-items: center; gap: 8px;
...
.sb-action.danger    { background: rgba(239,68,68,.1); color: #f87171; }
.sb-action.danger:hover { background: rgba(239,68,68,.2); }
...
#saveStatus { font-size: 10px; color: #334155; }
...
/* ── Floating toolbars ── */
.editor-popup { position:fixed; z-index:9999; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,.5),0 0 0 1px rgba(255,255,255,.08); backdrop-filter:blur(18px); -webkit-backdrop-filter:blur(18px); padding:5px 7px; display:flex; align-items:center; gap:3px; transition:opacity .12s,transform .12s; transform-origin:top left; }
.editor-popup.hidden { display:none; }
.editor-popup, .editor-popup.theme-dark-bg { background:rgba(13,17,23,.95); color:#f1f5f9; border:1px solid rgba(255,255,255,.1); }
.editor-popup.theme-light-bg { background:rgba(255,255,255,.97); color:#0f172a; border:1px solid rgba(0,0,0,.1); }
.tb-btn { width:28px; height:28px; border-radius:7px; border:none; background:transparent; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:12px; transition:background .12s,color .12s; }
.editor-popup.theme-light-bg .tb-btn { color:#0f172a; }
.editor-popup .tb-btn { color:#f1f5f9; }
...
.editor-popup.theme-light-bg .tb-sep { background:rgba(0,0,0,.1); }
...
#imageToolbar { width:210px; flex-direction:column; align-items:stretch; gap:8px; padding:10px; }
...
.dropzone  { border:2px dashed rgba(99,102,241,.4); border-radius:9px; padding:10px; text-align:center; cursor:pointer; font-size:11px; transition:border-color .15s,background .15s; color:inherit; opacity:.7; }
...
<div id="editorShell">
  ...
  <div id="editorSidebar">
    ...
    <div id="sbTools">
      ...
      <button type="button" class="tool-btn" id="toolHintToggle">
        <i class="fa-regular fa-circle-question"></i> How to edit
        <span class="tool-hint">?</span>
      </button>
    ...
    <!-- Help hint (toggleable) -->
    <div id="sbHint">
      <strong>Click text</strong> to edit &nbsp;&bull;&nbsp;
      <strong>Click image</strong> to replace &nbsp;&bull;&nbsp;
      <strong>Click section</strong> to recolour &nbsp;&bull;&nbsp;
      <strong>Drag <i class="fa-solid fa-grip-lines"></i></strong> to reorder sections
    </div>

    <!-- Actions -->
    <div id="sbBottom">
      <span id="saveStatus" style="font-size:10px;color:#334155;text-align:center;"></span>
  ...
  <div id="editorMain">
    ...
    <div id="editorTopBar">
      ...
      <span id="saveStatusTop" class="text-xs" style="color:#334155;font-size:10px;"></span>
    </div>
    ...
    <div id="editorViewport">
      ...
      <!-- Responsive preview buttons (floating bottom-center) -->
      <div id="previewBar">
...
document.querySelectorAll('.preview-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    ...
    // reset inline styles from transitions
    inner.style.cssText = '';
  });
});

// Sync sidebar undo/redo buttons with the top bar ones
// (site_editor.js controls #undoBtn / #redoBtn; we mirror to sidebar)
...
// Patch: site_editor.js targets #undoBtn / #redoBtn / #saveStatus
// We intercept disabled state changes via MutationObserver to keep both in sync
...
// Forward top bar button clicks back to the sidebar buttons (which site_editor.js controls)
