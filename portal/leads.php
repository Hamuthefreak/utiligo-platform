...
$quota_used = 0; $quota_limit = defined('FREE_SEARCH_DAILY_LIMIT') ? (int)FREE_SEARCH_DAILY_LIMIT : 2; $quota_resets_at = null;
if (!$is_pro) {
    try {
        $pdo         = get_platform_db();
...
<div class="glass rounded-2xl p-5 mb-8 border border-white/5">
  <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
    <div class="flex items-center gap-2">
      <div class="w-8 h-8 rounded-xl bg-emerald-500/15 flex items-center justify-center"><i class="fa-solid fa-magnifying-glass text-emerald-400 text-xs"></i></div>
    ...
    <div class="flex items-center gap-3">
      <div class="flex items-center gap-1.5 <?= $quota_remaining===0?'bg-red-500/10 border border-red-500/20 text-red-400':($quota_remaining===1?'bg-amber-500/10 border border-amber-500/20 text-amber-400':'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400') ?> rounded-full px-3 py-1 text-xs font-bold">
        ...
        <?php if($quota_remaining===0): ?>No searches left today<?php else: ?><?= $quota_remaining ?> search<?= $quota_remaining!==1?'es':'' ?> left today<?php endif; ?>
      </div>
  ...
  <div id="lockedWrap" class="hidden mt-4">
    ...
    <div class="mt-6 rounded-2xl overflow-hidden border border-emerald-500/20" style="background:linear-gradient(135deg,#0c1f16 0%,#0a1520 100%)">
      <div class="px-6 pt-7 pb-3 text-center">
        <div class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-emerald-500/15 border border-emerald-500/25 mb-4"><i class="fa-solid fa-lock text-emerald-400 text-2xl"></i></div>
        <h3 class="text-lg font-bold text-white mb-2">More Leads Are Waiting</h3>
        <p class="text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">You&rsquo;re seeing <strong class="text-white"><?= defined('FREE_LEAD_LIMIT')?(int)FREE_LEAD_LIMIT:3 ?> of the top results.</strong> Upgrade to unlock every lead, phone numbers, and instant website generation.</p>
      </div>
      ...
      <div class="px-6 pb-6 flex flex-col sm:flex-row items-center justify-center gap-3">
        <a href="/portal/billing.php?upgrade=1" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-400 active:scale-95 text-slate-950 px-8 py-3 rounded-xl font-bold text-sm text-center transition-all"><i class="fa-solid fa-crown mr-2"></i>Upgrade to Pro</a>
