...
<div class="space-y-3 mb-8">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
    <div class="glass rounded-xl p-3 border border-white/5 text-center"><p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Leads / search</p><p class="text-2xl font-extrabold text-white"><?= $FREE_LEAD_LIMIT ?></p></div>
    <div class="glass rounded-xl p-3 border border-white/5 text-center"><p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Searches / day</p><p class="text-2xl font-extrabold text-white"><?= $FREE_SEARCH_LIMIT ?></p></div>
    <div class="glass rounded-xl p-3 border border-white/5 text-center"><p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Sites / day</p><p class="text-2xl font-extrabold text-white"><?= $FREE_GEN_LIMIT ?></p></div>
    <div class="glass rounded-xl p-3 border border-white/5 text-center"><p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Templates</p><p class="text-2xl font-extrabold text-white"><?= $FREE_TMPL_LIMIT ?></p></div>
  </div>
  ...
  <div class="glass rounded-2xl p-5 border border-white/5">
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
      ...
      <span id="leadUpgradeBtn" class="<?= $ll_pct<80?'hidden':'' ?>">
        <a href="/portal/billing.php?upgrade=1&plan=entrepreneur" class="text-xs bg-white hover:bg-slate-200 text-black px-3 py-1.5 rounded-full font-bold"><i class="fa-solid fa-rocket mr-1"></i>Upgrade</a>
      </span>
    </div>
    <div class="w-full bg-white/5 rounded-full h-2 overflow-hidden">
      <div id="leadLimitBar" class="h-2 rounded-full transition-all <?= $ll_col ?>" style="width:<?= $ll_pct ?>%" data-used="<?= $pro_lead_count ?>" data-limit="<?= $PRO_LEAD_LIMIT ?>"></div>
    </div>
...
<?php else: ?>
<?php $sl_pct=$ENT_SITE_LIMIT>0?min(100,round(($active_site_count/$ENT_SITE_LIMIT)*100)):0; $sl_col=$sl_pct>=90?'bg-red-500':($sl_pct>=70?'bg-amber-500':'bg-white/60'); ?>
<div class="grid sm:grid-cols-2 gap-3 mb-8">
  <div class="flex items-center gap-3 glass rounded-2xl px-5 py-4 border border-white/5"><i class="fa-solid fa-infinity text-white text-xl"></i><div><p class="text-sm font-semibold text-white">Unlimited Lead Searches</p><p class="text-xs text-slate-400">Entrepreneur plan &mdash; no cap</p></div></div>
  <div class="glass rounded-2xl p-5 border border-white/5">
    <div class="flex items-center gap-2 mb-3"><div class="w-8 h-8 rounded-xl bg-white/8 flex items-center justify-center"><i class="fa-solid fa-globe text-slate-300 text-xs"></i></div><div><p class="text-sm font-semibold text-white">Active Sites</p><p class="text-xs text-slate-400"><?= $active_site_count ?> of <?= $ENT_SITE_LIMIT ?> used</p></div></div>
    <div class="w-full bg-white/5 rounded-full h-2 overflow-hidden"><div class="h-2 rounded-full transition-all <?= $sl_col ?>" style="width:<?= $sl_pct ?>%"></div></div>
  ...
  <form id="leadSearchForm">
    <div class="grid sm:grid-cols-3 gap-3 mb-5">
      <div>
        ...
        <div class="relative"><i class="fa-solid fa-city absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs"></i>
          <input type="text" name="city" id="fieldCity" placeholder="e.g. Calgary" required
                 class="w-full bg-slate-800/80 border border-slate-600 text-white placeholder-slate-500 rounded-xl pl-8 pr-4 py-2.5 text-sm focus:outline-none focus:border-white/40 transition-colors"></div>
      </div>
      <div>
        ...
        <div class="relative"><i class="fa-solid fa-briefcase absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs"></i>
          <input type="text" name="industry" id="fieldIndustry" placeholder="e.g. Plumber, Dentist" required
                 class="w-full bg-slate-800/80 border border-slate-600 text-white placeholder-slate-500 rounded-xl pl-8 pr-4 py-2.5 text-sm focus:outline-none focus:border-white/40 transition-colors"></div>
      </div>
      <div>
        <label class="block text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1.5">Keywords <span class="text-slate-600 normal-case font-normal text-[10px]">(optional)</span></label>
        <div class="relative"><i class="fa-solid fa-tags absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs"></i>
    ...
    <div class="flex flex-col sm:flex-row sm:items-end gap-4">
      <div class="flex-1">
        <label class="flex justify-between text-xs text-slate-500 font-semibold uppercase tracking-wider mb-2">
          <span>How many leads?</span><span class="text-white font-bold" id="leadCountDisplay"><?= $slider_default ?></span>
        </label>
        <input type="range" id="leadCountSlider" name="lead_count_slider" min="1" max="<?= $slider_max ?>" value="<?= $slider_default ?>" class="w-full accent-white h-2 cursor-pointer">
      ...
      <div class="flex flex-col gap-1 pb-0.5">
        <label class="flex items-center gap-2.5 cursor-pointer group">
          <input type="checkbox" id="includeSeenLeads" name="include_seen" class="w-4 h-4 rounded border-slate-600 bg-slate-800 accent-white cursor-pointer">
          <span class="text-xs text-slate-400 group-hover:text-white transition">Include already-seen leads</span>
        </label>
        <p class="text-[10px] text-slate-600 pl-6"><i class="fa-solid fa-circle-info mr-1"></i>Stored locally &mdash; never counts against your limit</p>
      </div>
      <button type="submit" class="inline-flex items-center gap-2 bg-white hover:bg-slate-200 active:scale-95 text-black px-7 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap">
