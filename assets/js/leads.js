document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderLeadRow(lead, seenIdsBefore) {
    ...
    // lead.id is now the integer DB id — carry-over to generate.php works
    row.innerHTML = `
  ...
  // ─ History sidebar — nav-link style matching left sidebar ────────────────────
  ...
  function renderHistoryItem(entry) {
    // Matches .nav-link CSS from portal_layout.php exactly
    ...
    // nav-link styles: flex, gap-10px, padding 10px 14px, border-radius 12px, font-size .875rem
    item.innerHTML = `
      <div class="nav-link flex-col items-start gap-0 hover:bg-white/6 cursor-default" style="padding:10px 14px;border-radius:12px;">
        <div class="flex items-center justify-between w-full gap-2">
          <span class="text-sm font-semibold text-white truncate flex-1">${escHtml(entry.city)}</span>
          ${entry.result_count>0?`<span class="text-xs font-bold text-slate-500 shrink-0">${entry.result_count}</span>`:''}
        </div>
        <span class="text-xs text-slate-400 truncate w-full mt-0.5">${escHtml(entry.industry)}${entry.keywords?' · '+escHtml(entry.keywords):''}</span>
        <div class="flex items-center justify-between w-full mt-1.5">
          <span class="text-[10px] text-slate-600">${mo} · ${time}</span>
          <button type="button"
            data-city="${escHtml(entry.city)}" data-industry="${escHtml(entry.industry)}" data-keywords="${escHtml(entry.keywords||'')}"
            class="view-btn opacity-0 group-hover:opacity-100 inline-flex items-center gap-1 text-[10px] font-bold text-slate-300 hover:text-white bg-white/8 hover:bg-white/15 px-2 py-0.5 rounded-lg transition-all">
            <i class="fa-solid fa-arrow-up-right-from-square text-[8px]"></i> View
          </button>
        </div>
      </div>`;
  ...
  function runSearch(city, industry, keywords, leadCount, includeSeen, forceRefresh) {
    ...
    .then(data => {
      ...
      // Always sync both counters — every response now includes both fields
      ...
      // BUG A/B FIX: snapshot before marking seen
