 ...
 * Usage: replace `require_once __DIR__ . '/../includes/header.php';` with this file in portal pages.
 * Pages that still use header.php directly (public pages, auth pages) are unaffected.
 */
...
<style>
  ...
  /* Sidebar on mobile: slide-in drawer */
...
<aside id="sidebar" class="w-64 h-screen bg-slate-900/95 border-r border-white/5 flex flex-col lg:fixed lg:top-0 lg:left-0 backdrop-blur-xl">
  ...
  <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
    ...
    <a href="/portal/index.php"    class="nav-link <?= _nav_active('/portal/index.php',    $_path) ?>">
      <i class="fa-solid fa-house"></i> Dashboard
    </a>
    <a href="/portal/leads.php"    class="nav-link <?= _nav_active('/portal/leads.php',    $_path) ?>">
      <i class="fa-solid fa-magnifying-glass"></i> Find Leads
    </a>
    <a href="/portal/generate.php" class="nav-link <?= _nav_active('/portal/generate.php', $_path) ?>">
      <i class="fa-solid fa-bolt"></i> Generate Site
    </a>
    <a href="/portal/my_sites.php" class="nav-link <?= _nav_active('/portal/my_sites.php', $_path) ?>">
      <i class="fa-solid fa-folder-open"></i> My Sites
    </a>

    <p class="text-xs font-semibold text-slate-600 uppercase tracking-widest px-3 mt-5 mb-2">Account</p>

    <a href="/portal/settings.php" class="nav-link <?= _nav_active('/portal/settings.php', $_path) ?>">
      <i class="fa-solid fa-paintbrush"></i> White-Label
    </a>
    <a href="/portal/billing.php"  class="nav-link <?= _nav_active('/portal/billing.php',  $_path) ?>">
      <i class="fa-solid fa-credit-card"></i> Billing
    </a>
    <?php if (($_user['admin_flag'] ?? 0) && (defined('DEBUG_MODE') && DEBUG_MODE)): ?>
    <a href="/portal/debug.php" class="nav-link <?= _nav_active('/portal/debug.php', $_path) ?>">
  ...
  <div class="px-4 py-4 border-t border-white/5 flex items-center gap-3">
    ...
    <a href="/includes/auth.php?action=logout" title="Logout"
...
<header class="lg:hidden sticky top-0 z-30 bg-slate-950/90 backdrop-blur border-b border-white/5 px-4 py-3 flex items-center justify-between">
  ...
  <a href="/portal/index.php" class="flex items-center gap-2 font-black text-base">
