...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_config'])) {
    if (!admin_csrf_verify('config', $_POST['csrf_token'] ?? null)) {
        ...
        foreach ($fields as [$key, $label, $type]) {
            ...
            switch ($type) {
                ...
                case 'float':
                    ...
                    break;
                default:
                    $ival = $type === 'int' ? max(0, (int)$raw) : (int)$raw;
        ...
        if (@file_put_contents($overrides_file, $php) !== false) {
            ...
            $save_error = 'Could not write storage/config_overrides.php — check directory permissions.';
        }
    }
}
...
<div class="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-2xl px-5 py-4 mb-6 text-sm">
  <i class="fa-solid fa-circle-check"></i> Settings saved — all changes are live.
</div>
<?php endif; ?>
<?php if ($save_error): ?>
...
<form method="POST" action="/admin/settings.php">
  ...
  <div class="bg-white/[0.03] hover:bg-white/[0.045] border border-white/5 hover:border-white/10 rounded-2xl p-6 mb-4 transition-all">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
      ...
      <div>
        ...
        <?php if ($hint): ?><p class="text-[11px] text-slate-600 mb-1.5"><?= htmlspecialchars($hint) ?></p><?php endif; ?>
        <?php if ($type === 'bool'): ?>
        ...
        <?php elseif ($type === 'float'): ?>
          <div class="relative">
            ...
            <input type="number" step="0.01" min="0" name="cfg[<?= $key ?>]" value="<?= htmlspecialchars((string)$cur) ?>"
                   class="w-full bg-slate-900/70 border border-white/10 focus:border-purple-500/50 text-white rounded-xl pl-7 pr-3 py-2.5 text-sm outline-none transition">
          </div>
        <?php else: ?>
          <input type="number" step="1" <?= $type === 'int' ? 'min="0"' : 'min="-1"' ?>
                 name="cfg[<?= $key ?>]" value="<?= htmlspecialchars((string)$cur) ?>"
                 class="w-full bg-slate-900/70 border border-white/10 focus:border-purple-500/50 text-white rounded-xl px-3 py-2.5 text-sm outline-none transition">
        <?php endif; ?>
      </div>
      <?php endforeach; ?>
    </div>
  ...
  <div class="sticky bottom-4 z-20 flex items-center justify-between bg-slate-900/90 backdrop-blur border border-white/10 rounded-2xl px-6 py-4 mt-2 shadow-xl">
    <p class="text-xs text-slate-500 hidden sm:block">Saved values override all code defaults instantly.</p>
    <button type="submit" class="ml-auto inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-500 active:scale-95 text-white px-7 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-purple-900/30">
